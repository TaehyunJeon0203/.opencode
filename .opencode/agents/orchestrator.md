...
Typical feature: explore only if needed -> architect if non-trivial -> coder -> tester -> reviewer when risk warrants. Typical difficult bug: explore only if needed -> debugger -> coder -> tester -> reviewer.
